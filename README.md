...
├── dataset/
│   └── tmall-buy-merged/           # Directory structure for the Tmall dataset.
│                                     # Note: Large data files are not included due to size limits.
...
├── saved/                          # This directory will store trained model checkpoints.
...
- **`dataset/`**: Holds the datasets. Due to GitHub's file size limits, large data files (like `.inter` files) are not included. The RecBole framework will process the raw data and generate them during the first run. The `amazon-book-23` and `yelp-2021` datasets will also be automatically downloaded by RecBole.
...
- **`saved/`**: This directory is initially empty. It will store the pre-trained model checkpoints (`.pth` files) after you run the training script (`mf.py`).
...
#### Step 1: Train the Base Model (BPR)
